...
export default function LoginLogs() {
  ...
  const rows: LoginLogRow[] = logs.map((log) => ({
    ...
    failure_reason: log.failure_reason
      ...
      : "--",
    user_agent: (
      <span title={log.user_agent} className="stc-ellipsis-cell">
        {log.user_agent || "--"}
      </span>
    ),
        ...
        <div className="yza-doc-stack">
          {isLoading ? (
            ...
            <EmptyTable
              ...
              description={hasFilters ? t("common.emptyFiltered") : t("loginLogs.emptyDescription")}
              headers={columns.map((column) => column.header)}
              heading={t("loginLogs.emptyTitle")}
            />
          ) : (
            <>
