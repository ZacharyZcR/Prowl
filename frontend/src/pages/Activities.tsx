...
export default function Activities() {
  ...
  async function handleExport() {
    ...
    try {
      ...
      const requestUrl = new URL(buildApiUrl("/api/v1/activities/export"));
      ...
      toast.error(e instanceof Error ? e.message : t("data.exportFailed"));
    } finally {
      setExporting(false);
    }
  }
    ...
    <PageShell
      ...
      actions={
        <Button
          ...
          aria-busy={exporting}
        >
          {exporting ? t("activities.exporting") : t("activities.export")}
        </Button>
      }
    >
      <KPIGroup
        ...
        <div className="yza-doc-stack">
          {isLoading ? (
            ...
            <EmptyTable
              ...
              heading={t("activities.emptyTitle")}
            />
          ) : (
            <>
      ...
      >
        {selected && (
          <div className="yza-doc-stack stc-activity-detail">
            ...
            <DetailRow label={t("activities.time")} value={formatDateTime(selected.created_at)} />
            {selected.details ? (
              <div>
                <div className="stc-activity-detail__label">{t("activities.detailData")}</div>
                <pre className="stc-activity-detail__pre">
...
function DetailRow({ label, value }: { label: string; value: ReactNode }) {
  return (
    <div className="stc-detail-row">
      <span className="stc-detail-row__label">
        {label}
      </span>
      <span className="stc-detail-row__value">{value}</span>
    </div>
  );
}
