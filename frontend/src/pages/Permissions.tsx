...
import { DataTable, KPIGroup, Skeleton, Alert, Button, EmptyState, Tag } from "@yza/ui";
...
function toRow(p: Permission): PermRow {
  return {
    code: <code className="stc-inline-code stc-inline-code--mono">{p.code}</code>,
    name: p.name,
  ...
  return (
    ...
    >
      ...
      {isLoading ? (
        ...
        <section className="yza-doc-card">
          <EmptyState
            heading={t("permissionsPage.emptyTitle")}
          ...
          <section key={cat.category} className="yza-doc-card">
            <div className="yza-doc-stack">
              <div>
                ...
                <p className="yza-doc-card__meta">{t("permissions.nPermissions", { n: cat.permissions.length })}</p>
              </div>
              <DataTable<PermRow>
                columns={columns}
