...
export const Input = forwardRef<HTMLInputElement, InputProps>(
  ({ label, hint, message, leading, status = "default", className, id, disabled, ...rest }, ref) => {
    const inputId = id ?? label?.toLowerCase().replace(/\s+/g, "-");
    return (
      <div className="flex flex-col gap-1">
        {label && (
          <label htmlFor={inputId} className="text-sm font-medium text-[var(--stc-text-primary)]">
            {label}
        ...
        >
          ...
          <input
            ...
            id={inputId}
            disabled={disabled}
            className="w-full bg-transparent text-[var(--stc-text-primary)] placeholder:text-[var(--stc-text-tertiary)] outline-none"
            {...rest}
          />
        </div>
