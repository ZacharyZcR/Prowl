...
import { Button, Drawer, EmptyState, Pagination, Skeleton, Tabs, Tag } from "@yza/ui";
...
function NotificationList({ unreadOnly }: { unreadOnly: boolean }) {
  ...
  const { data, isLoading } = useNotifications({ page, unread_only: unreadOnly || undefined });
    ...
    <div className="stc-notification-list">
      {items.map((n) => (
        ...
        >
          ...
          {!n.read && (
            <Button size="sm" tone="neutral" onClick={() => markRead.mutate(n.id)}>
    ...
    <>
      ...
      {createPortal(
        <Drawer
          ...
          footer={
            <Button size="sm" tone="outline" onClick={() => markAllRead.mutate()}>
